...
const LESSONS: LessonType[] = [
  {
    ...
    slides: [
      ...
      },
      {
        ...
        content: '<div class="bloc1"><p>📉 Do you find it difficult to keep up with rising costs?</p><hr><p>Scarcity in Bitcoin refers to there being a cap on how many Bitcoins can ever exist: a maximum of 21 million. This <code>max supply</code> is embedded in its code, making Bitcoin rare, much like gold.</p><p>While the max supply of <code>BTC</code> can’t exceed 21 million, new currency is still added to the <code>circulating supply</code>. Much like mining gold, new Bitcoin is introduced through a laborious process called <code>cryptocurrency mining</code>. Roughly every four years, this new issuance is cut in half (the “halving”), shrinking toward zero as the cap approaches. Over time, miners rely more on transaction fees than on new coins.</p><p>Bitcoin\'s fixed <code>monetary policy</code> makes it attractive for preserving value in the long term. Since 2024, regulated funds (spot ETFs) have let traditional investors hold Bitcoin, strengthening its “digital gold” role.</p><p>Its short term value remains volatile due to market speculation.</p></div><div class="bloc2"><img src=\'/images/bitcoin-basics/digital-gold-scarcity-8c8905f7.svg\'></div>'
      },
      {
        type: 'QUIZ',
  ...
  },
  {
    ...
    slides: [
      ...
      },
      {
        ...
        content: '<div class="bloc1"><p>⚖️ <strong>Wouldn’t the internet be a better place if the same rules applied to everyone?</strong></p><hr><p>Think of Ethereum as a digital city, where everything (from businesses to public services) is run by self-service, transparent systems. There are no intermediaries. With public-facing programming, users can easily review their trade terms and even inspect how the system works.</p><p>Transactions in the city are noted on a public record called the <code>Ethereum blockchain</code>. The blockchain tracks and stores digital property in the city, removing the need for another classic intermediary: the bank!</p><p>Every transaction pays a small fee. Most of that fee is destroyed, which can make the city’s currency, <code>ether</code>, scarcer as the city gets busier. A small tip rewards the computer that processes your transaction. And the city’s rules aren’t set by any company: changes are agreed on openly by the builders, researchers, and users who care for it.</p><p>On Ethereum, transparent code has replaced kings. Nobody is above the rules.</p></div><div class="bloc2"><img src=\'/images/ethereum-basics/ethereum-the-open-internet-ab794154.svg\'></div>'
      },
      {
        type: 'LEARN',
  ...
  },
  {
    ...
    slides: [
      ...
      },
      {
        ...
        content: '<div class="bloc1"><p>In traditional markets and <code>CEXs</code>, your custodian uses an order book: a database filled with buy and sell offers. The CEX connects your trade offer with another person’s. You are usually charged a base/scaling commission on your trade, and you’re also left wondering if the undisclosed matching method found you the best possible deal.</p><p>Most <code>DEXs</code> use ‘Automated Market Maker’ (<code>AMM</code>) technology, the most common design for token swaps: a system that prices your trade using a public algorithm. Some newer DEXs use order books or intent-based systems instead. As the AMM algorithm is open-source, anyone can understand, clone, and improve it, leading to healthy competition and constant innovation.</p><p>AMMs route user trades through <code>liquidity pools</code>, rather than directly matching user bids and asks. These communal token vaults accumulate and dispense tokens according to user interactions, with every step visible on the public blockchain.</p></div><div class="bloc2"><img src=\'/images/decentralized-exchanges/automated-market-makers-78ad7439.svg\'></div>'
      },
      {
        type: 'QUIZ',
  ...
  },
  {
    ...
    slides: [
      ...
      },
      {
        ...
        title: 'Slashing',
        content: '<div class="bloc1"><p>When a node provably cheats, like signing two conflicting blocks, its stake is <code>slashed</code>: it loses a chunk of its Ether. Being slow or offline is not slashing; that only costs a small penalty. This way, staked Ether becomes a security deposit where nodes are incentivized to be honest and keep Ethereum running smoothly.</p><p>For an attacker to approve fraud on Ethereum, they must have control over majority of the validator nodes and thus, staked Ether. Fortunately this can\'t happen if everyday users are involved in staking. <code>Decentralized</code> ownership of the network divides the power to make decisions.</p></div><div class="bloc2"><img src=\'/images/staking-on-ethereum/slashing-ccabe137.svg\'></div>'
      },
      {
        type: 'QUIZ',
