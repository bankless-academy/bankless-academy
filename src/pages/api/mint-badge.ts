...
export default async function handler(
  ...
  try {
    ...
    try {
      ...
      // ERC-8021 Builder Code attribution suffix (bc_6vclforu)
      const BUILDER_CODE_SUFFIX = '62635f3676636c666f72750b0080218021802180218021802180218021'

      let mint;
      try {
        const populatedTx = await contract.populateTransaction.multicall(multicallData, options)
        populatedTx.data = populatedTx.data + BUILDER_CODE_SUFFIX
        mint = await signer.sendTransaction(populatedTx)
        // // simulate mint
        // mint = {
