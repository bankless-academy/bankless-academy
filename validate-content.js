...
const langKeywordIndex = (lang) => {
  ...
  for (const [englishKey, e] of Object.entries(bundle))
    for (const f of [
      ...
      if (typeof f === 'string' && f) forms.add(normalizeKeyword(f))
  ...
  const englishKeyed = keys.filter((k) =>
    keywordKeys.has(normalizeKeyword(k))
  ).length
  const current =
...
// Body splitter, needed by both the English loop below and the translated-md
// loop further down, so it is declared before either of them.
const SPLIT = '```\n\n---'
...
for (const file of files) {
  ...
  for (const b of findBrokenEmphasis(bodyOf(md)))
    err(
      `line ${b.line}: emphasis marker will not render (${b.kind}) — ${b.text.slice(0, 60)}`
    )
...
for (const [slug, m] of Object.entries(meta)) {
  ...
  for (const lang of langs) {
    ...
    // Emphasis that CommonMark refuses to close ships as a literal ** to the
    // reader. Only CJK triggers it, but the check is cheap enough to run
    // everywhere rather than maintain a language list.
    {
      const broken = findBrokenEmphasis(body)
      if (broken.length) {
        errors.push(
          `${lang}/${slug}: ${broken.length} line(s) with emphasis markers that will not render (${[
            ...new Set(broken.map((b) => b.kind)),
          ].join(', ')}) — first at line ${broken[0].line}: ${broken[0].text.slice(0, 60)}`
        )
      }
    }
