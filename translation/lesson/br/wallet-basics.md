...
DESCRIPTION: O que é uma carteira de criptomoedas? Learn how wallets work and create your first one today!
LANGUAGE: Português
...
Nesta lição, falaremos sobre carteiras, como elas funcionam e como você pode manter suas criptomoedas seguras. We’ll end with a guide on how to set one up and access **the Ethereum blockchain, the home of Bankless Academy.**
...
O mesmo não acontece com as carteiras de criptomoedas. Because of how blockchains work, wallets are assigned an “address” and a complex “password” when they’re created, and you do not choose these.

This address and password combination cannot be changed, although you can always create a new wallet if you need to. Some newer wallets add recovery features, but never rely on that. Vamos aprender um pouco mais sobre esses endereços e senhas especiais, para que nos possamos sentir confiantes ao usá-los!
...
If your address is like a house address, then the `private key` is like the key to the front door. É a "senha" para seus fundos e ativos.
...
> ℹ️ Correto! Your address lets others find you, while only your private key lets you in.
...
Your recovery phrase is like a master key: it can recreate every key pair in your wallet. **Nunca compartilhe sua frase de recuperação.**
...
Each wallet serves a different use-case. Não se preocupe, criaremos apenas uma carteira hoje.
...
Os serviços de `carteira custodial` incluem corretoras de criptomoedas, como Coinbase e Kraken. **They are your wallet custodian**: they look after your private keys for you. Você acessa seus serviços como qualquer outro site (com um login de e-mail e uma senha redefinível).
...
A `hot wallet` is a wallet accessed via an app on your device, the Zerion app for example. Tal como muitos aplicativos da `web2`, um aplicativo de carteira está sempre conectado à internet.
...
🖊️ Record your `recovery phrase` on durable physical material (laminated paper, or even steel) and store it somewhere very safe. Nunca compartilhe esse material ou a sua `chave privada` com ninguém. **It’s more than just a password.**
...
> ℹ️ Tente novamente! A single password manager breach could expose it. Keep your phrase offline.
...
> ℹ️ Correto! Zerion wallets are self-custody wallets, controlled by you, and you alone.
