...
- [x] **`zh-tw` shipped as a DERIVED language (2026-08-22):** Traditional Chinese generated from the finished `zh` by `convert-zh-tw.js` (new, repo root) — OpenCC cn→twp plus a corpus-adjudicated override table. 21 lessons (19 active + the 2 deprecated, parity with `zh`), 274-entry glossary, 4 UI namespaces, style guide with 18 gate-checked pins, registered in `languages[]` (`lang-tools register` skips deprecated lessons — those two were added by hand) and `LAZY_RESOURCES`. Structural parity is free: a conversion cannot change unit counts or `[x]` positions, and every backticked term matches its glossary keyword because both run through the same pipeline. Of the 100 glossary terms ETHGlossary zh-tw covers, **94 agreed with the raw conversion**; the overrides encode the adjudicated rest plus a full corpus scan: `數字→數位` (digital) with the three number-sense phrases protected, `通過→透過` (instrumental) with the four pass/approve phrases protected, `數位簽章`, `智能合約/錢包/帳戶` (ETHGlossary citation forms — twp's `智慧X` is only an alias), `帳` never `賬`, `存取`, `品質`, `提案者`, `對等節點`, `列印`; kept as-is where TW usage agrees: `文件` (documentation), `程序` (procedure), `支持者`, and Latin `Gas`/`Blob`/`gwei`. The protect lists are corpus-specific: the script prints their hit counts each run, and NEW zh prose using 數字-as-digits or 通過-as-pass in an unlisted phrase needs the lists extended. **The standing rule: never hand-edit zh-tw and never run a translation wave for it — fix `zh` (or the override table) and re-run the converter.**
- [x] **nl / th / tl wave complete (2026-08-22):** Dutch, Thai and Filipino shipped end to end — 274-entry glossary, 4 UI namespaces and 19 lessons each, registered in `languages[]` and `LAZY_RESOURCES`. **24 non-English languages now live.** Three firsts, each with a lesson:
  - **First wave with NO vendored ETHGlossary data** (upstream stops at the same 24 languages; `am` will hit this too — `lang-tools pins` now degrades gracefully). The style guides carried ALL terminology authority: 121 pins each plus fixed headings/openers/True-False/credit labels pinned up front. Result: **nl and th shipped with zero cross-file consistency warnings**; tl needed only 3 heading-casing normalizations. Pin everything recurring before any agent runs — it works.
  - **First wave on Sonnet subagents (medium effort, via the `translator` agent type in `~/.claude/agents/`)** after two usage-limit strikes killed 10+ Fable agents mid-wave. The `.translation-drafts/` discipline made both strikes nearly free: a complete th glossary half and 7 finished lesson bodies were salvaged with zero retranslation. Sonnet quality held with ONE systematic correction: the pilot shipped 12/14 slide headings verbatim English (the heading-ratio warning caught it; Fable agents got this right unprompted). Fix: an explicit heading rule + a benchmark file in every brief. Live rate-limit meters now come from `~/.claude/check-usage.sh` (OAuth usage endpoint) + a SessionStart hook; launches are meter-gated with **max 2 agents in flight** (user rule).
  - **Recurring backtick slips worth a future lint**: th agents split pinned แบบ/การ prefixes outside the backticks (dead tooltip), tl agents pulled `mga` inside them — both self-corrected via the dead-tooltip check, both recurred across agents.
- [x] **Arabic (`ar`) wave complete (2026-08-22)** — first RTL language, 25 non-English now in the registry. 274-entry glossary (ETHGlossary ar EXISTS upstream, unlike wave 6 — pins still overrode it where usage wins: بلوكتشين over سلسلة الكتل, خزنة over قبو, Blob kept Latin since their كتلة بيانات collides with block = كتلة), 4 UI namespaces, 19/19 lessons, all gates green. Arabic-specific trap class encoded in `translation/style/ar.md` "Script mechanics": NO diacritics anywhere (a diacritized keyword never matches an undiacritized backtick), one hamza orthography, and — derived from the RTL work — **the keyword span's `inline-block` breaks Arabic cursive joining, so attached ف/ب/ل/ك prepositions and pronoun suffixes must never touch a backticked term** (rephrase with free prepositions; و is safe outside backticks since it never joins leftward). One real merge collision (hash + sharding both تجزئة → sharding re-pinned تجزئة الشبكة); two `_`-flanking emphasis errors caught by the build gate. Mid-wave central glossary fixes (oblique ون→ين plurals, missing definite forms) cut later groups' rephrasing friction — do that from the first group's flags in future Arabic-script waves (`ur` next, same playbook + this guide as base). **UI decision from the first real-Arabic review: content and navigation mirror under RTL; floating utility chrome does NOT** — the ChatWidget + Helper badge are pinned physical bottom-right in all languages (industry convention; docs/rtl-audit.md C5).
- [x] **RTL support implemented (2026-08-22)** — the one-time UI gate for `ar`/`ur`. Full findings, the measured Chakra prop-resolution matrix, and per-phase decisions in `docs/rtl-audit.md` (keep that doc current if any of this moves). The architecture: the Chakra theme stays LTR FOREVER (per-language theme recreation churns emotion class hashes and breaks the hard-coded `.css-XXXXXX` selectors in `_app.tsx`); direction comes from the DOM. `applyDocumentLanguage()` in the registry is the ONLY writer of `<html lang>/<dir>` (AppContext calls it on every route change + imperative switch; it used to live in LanguageSelector, which unmounts with the nav on every lesson slide). New `_document.tsx` ships static defaults; the SSR /content pages carry `dir`+`lang` on the article root (crawler-visible, verified in served HTML with a temporarily-flipped registry). **The trap to remember**: Chakra logical props are TWO different things — `ms`/`me`/`ps`/`pe`/`borderInlineStart*`/`textAlign: start|end` pass through as real CSS logical properties (safe), but `insetInline*` and ALL logical corner-radius props (and Drawer/Popover `placement="end"`) resolve against the theme direction at build time, silently hardcoding LTR — those must use inline `style={{}}`, emotion CSS, an explicit `sx={{ '[dir="rtl"] &': …physical mirror… }}` block, or a runtime `isRtlDocument()` conditional. ~60 sites converted across 16 files; arrows mirror via a `.mirror-rtl` global class; lesson hotkeys swap at event time; keyword tooltips + `<code>` get `unicode-bidi: isolate` (detached trailing punctuation now stays inside the keyword's isolating span). Still open before `ar`/`ur` ship: the browser click-through in `docs/rtl-audit.md` Phase D — especially the Layout desktop rail (no inline inset, relies on document flow) and the OnboardingModal pill. (it 2, es 2, tr 1, pt-br 1, de 1 + older waves) — they now scroll rather than clip, and clear as each wave regenerates
